...
export default function ProductsCategory() {
  ...
  async function getProducts() {
    ...
    const filteredProds = data.data.filter(
            ...
            <div className="row justify-center items-center">
              {filteredProducts.length > 0 ? (
                filteredProducts?.map((product) => (
                  <div
                    key={product.id}
                    className="w-full md:w-1/2 md:px-5 lg:w-1/4 xl:w-1/5 md:p-4 group text-center"
                  >
                    <Helmet>
                ...
                <section className="text-center h-[60vh]">
                  <h3 className="md:text-3xl text-2xl py-24 font-bold text-green-600">
                    We couldn't find that Category . How about you check out our <Link to={'/'} className="underline hover:text-green-800 dark:hover:text-green-400">Home Page</Link>
                  </h3>
                </section>
              )}
            </div>
